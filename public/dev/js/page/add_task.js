$(function(){
    ...
    function sendData(afterfnSure){
        ...
        if (!base.formValidate($dockForm)) {
            ...
            $.ajax({
                url: "http://192.168.1.108:9211/task/insert",
                type: "POST",
                dataType: "json",
                data: sendData
            }).done(function(data){
                if(data.status == 1){
                    ...
                    window.open('http://192.168.1.108:9211/page/task_detail?id='+data.data);
    ...
    function beforeDialog(){
        ...
        var box = new Box({
            title: "添加特邀用户",
            html: "http://192.168.1.108:9211/page/task_add .add_task_form",
            css: {
                "min-width": "320px"
            },
